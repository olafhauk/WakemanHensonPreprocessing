...
mSSP_evo_svd_lats = [[0.05,0.15], [0.15,0.25],[0.25,0.5]] # latency windows for evoked SVD
...
def fname_mSSP_proj_tcs(C, subject, cov_latwin):

    cov_str = _cov_str(cov_latwin)

    fname_mSSP_svd = op.join(C.mSSP_SVD_dir, 'mSSP_proj_TCS_' + subject + '_' + cov_str + '.hdf')
...
def fname_mSSP_fig(C, sbj, string1, string2, suffix=''):
    ...
    fname_mSSP_fig = op.join(C.mSSP_fig_dir, string1 + '_' + str(sbj) + '_' + string2 + suffix)
