...
Based on WH_Res_ResolutionMetrics.py, but adapted to use
functionality from MNE-Development (mnedev) environment.
run WH_ResolutionMetrics_mnedev.py WH_Res_config 1
=========================================================

"""
# OH, November 2019
# modified using MNE 0.20, April 2020
...
def functions_and_metrics(resmat, src, functions, metrics, subject, stctext):
    ...
    for function in functions:
        ...
        for metric in metrics:
            ...
            resmet = resolution_metrics(resmat, invop['src'],
                                        function=function,
                                        metric=metric)
...
for sbj in sbj_ids:
    ...
    for params in paramlist:
        ...
        # info['sfreq'] = 1000.  # for writing results
        ...
        # # regularise empirical noise covariance matrix for LCMV beamformer
        # # similar to data covariance matrix when creating LCMV weights
        # noise_cov_lcmv = mne.cov.regularize(noise_cov_use, info, mag=0.05, grad=0.05,
        #                                     eeg=0.05, rank='info')
        ...
        # To compute "best" PSF and CTF,
        # find vertex with largest norm of leadfield column
        leadfield = fwd_use['sol']['data']  # leadfield for this configuration
        ...
        for (method, meth_name) in methods:
            ...
            if method == 'LCMV':  # for LCMV beamformer
                ...
                for cov_dat in cov_dats:
                    ...
                    # data_cov_use = mne.cov.pick_channels_cov(data_cov, ch_names)
                    ...
                    # lcmv_filters = make_lcmv(info, fwd_use, data_cov_use,
                    #                          reg=0.05, noise_cov=noise_cov_use,
                    ...
                    # noise_cov_lcmv = mne.cov.regularize(
                    #     noise_cov_lcmv, info, mag=0.1, grad=0.1, eeg=0.1,
                    #     rank='info')

                    ### EDIT: played with rank option
